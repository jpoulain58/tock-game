...
export interface TockGameState {
  pawns: Pawn[];
  deck: Card[];
  discard: Card[];
  players: Player[];
...
const TELEPORTERS: number[] = [];
const TELEPORTER_PAIRS: Record<number, number> = {};

export class TockGame {
  state: TockGameState;
  ...
  initGame(playerCount: number): TockGameState {
  ...
  getPawnAtHome(player: number, idx: number): Pawn | undefined {
    return this.state.pawns.find(p => 
  ...
  isPathBlocked(startIdx: number, steps: number, forward: boolean = true): boolean {
  ...
  applyTeleporter(pawn: Pawn): { teleported: boolean; capturedPawnId?: string } {
    if (pawn.location.type !== "RING") return { teleported: false };
    
    const currentIdx = pawn.location.idx;
    if (!TELEPORTERS.includes(currentIdx)) return { teleported: false };
  ...
  stepForward(pawn: Pawn, steps: number, playerSlot: number): { 
    success: boolean; 
    error?: string;
    finalLocation?: PawnLocation;
    passedThroughHome?: boolean;
  } {
  ...
  stepBackward(pawn: Pawn, steps: number, playerSlot: number): {
    success: boolean;
    error?: string;
    finalLocation?: PawnLocation;
  } {
  ...
  exitPawn(pawn: Pawn, playerSlot: number): {
    success: boolean;
    error?: string;
  ...
  getPawnAt(location: PawnLocation): Pawn | null {
    if (location.type === 'RING') {
      return this.getPawnAtRing(location.idx) || null;
    } else if (location.type === 'HOME') {

      const pawn = this.state.pawns.find(p =>
        p.location.type === 'HOME' && p.location.idx === location.idx
      );
      return pawn || null;
    }
    return null;
  ...
  canPlayAnyCard(playerSlot: number): boolean {
    const player = this.state.players[playerSlot];
    
    for (const card of player.hand) {
      
      if (card.rank === "J") {
        const pawnsOnBoard = this.state.pawns.filter(p => 
          p.location.type === "RING" || p.location.type === "HOME"
        );
        if (pawnsOnBoard.length >= 2) return true;
      }
      ...
      if (card.rank === "5") {
        ...
        if (pawnsOnBoard.length >= 1) return true;
      }

      if (card.rank === "A" || card.rank === "K") {
        const hasPawnInBase = this.state.pawns.some(p => 
          p.player === playerSlot && p.location.type === "BASE"
        );
        if (hasPawnInBase) {
          const startIdx = STARTS[playerSlot];
          const occupant = this.getPawnAtRing(startIdx);
        ...
        if (card.rank === "A") {
          const playerPawnsOnBoard = this.state.pawns.filter(p => 
            p.player === playerSlot && (p.location.type === "RING" || p.location.type === "HOME")
          );
          if (playerPawnsOnBoard.length > 0) return true; 
        }
      }
  ...
  passTurn(playerSlot: number, card?: Card): {
    success: boolean;
    events?: GameEvent[];
    error?: string;
  } {
  ...
  playCard(playerSlot: number, card: Card, action: any): { 
    success: boolean; 
    events?: GameEvent[]; 
    error?: string 
  } {
    ...
    if (card.rank === "A") {
      if (action.type === "exit") {
        ...
        if (!exitResult.success) return { success: false, error: exitResult.error };
    ...
    if (card.rank === "4") {
      ...
      if (!backResult.success) return { success: false, error: backResult.error };
    ...
    if (card.rank === "7") {
      ...
      const totalSteps = moves.reduce((sum: number, m: any) => sum + m.steps, 0);
      if (totalSteps !== 7) {
        return { success: false, error: "La somme des mouvements doit être 7" };
      }

      if (moves.some((m: any) => m.steps <= 0)) {
      ...
      for (const move of moves) {
        ...
        if (!pawn) return { success: false, error: `Pion ${move.pawnId} invalide` };
  ...
  private handleNormalMove(
    pawn: Pawn, 
    steps: number, 
    playerSlot: number, 
    events: GameEvent[],
    consumeCard: () => void
  ): { success: boolean; events?: GameEvent[]; error?: string } {
    if (pawn.location.type === "BASE") {
